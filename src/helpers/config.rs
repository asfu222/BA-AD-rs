use reqwest::header::{HeaderMap, HeaderValue};

pub const APK_DOWNLOAD_URL_REGEX: &str =
    r"(X?APKJ)..(https?://(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*))";
pub const APK_VERSION_REGEX: &str = r"(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)";

pub const GAME_CONFIG_PATTERN: &[u8] = &[
    0x47, 0x61, 0x6D, 0x65, 0x4D, 0x61, 0x69, 0x6E, 0x43, 0x6F, 0x6E, 0x66, 0x69, 0x67, 0x00, 0x00, 0x92, 0x03, 0x00, 0x00,
];

pub const PLAYSTORE_URL: &str = "https://play.google.com/store/apps/details?id=com.nexon.bluearchive&hl=in&gl=US";
pub const API_URL: &str = "https://api-pub.nexon.com/patch/v1.1/version-check";

pub const API_DATA_FILENAME: &str = "api_data.json";

#[derive(Clone)]
pub struct RegionConfig {
    pub id: String,
    pub version_url: String,
    pub apk_path: String,
    pub asset_filter: String,
}

impl RegionConfig {
    pub fn new(region: &str) -> Self {
        match region {
            "global" => Self {
                id: "global".to_string(),
                version_url: String::new(),
                apk_path: String::new(),
                asset_filter: String::new(),
            },
            _ => Self {
                id: "japan".to_string(),
                version_url: "https://api.pureapk.com/m/v3/cms/app_version?hl=en-US&package_name=com.YostarJP.BlueArchive"
                    .to_string(),
                apk_path: "apk/BlueArchive.xapk".to_string(),
                asset_filter: "assets/bin/Data/".to_string(),
            },
        }
    }

    pub fn catalogs_path(&self) -> String {
        format!("catalogs/{}", self.id)
    }

    pub fn catalog_file_path(&self, filename: &str) -> String {
        format!("catalogs/{}/{}", self.id, filename)
    }
}

pub fn http_headers() -> HeaderMap {
    let mut headers: HeaderMap = HeaderMap::new();
    headers.insert("x-cv", HeaderValue::from_static("3172501"));
    headers.insert("x-sv", HeaderValue::from_static("29"));
    headers.insert("x-abis", HeaderValue::from_static("arm64-v8a,armeabi-v7a,armeabi"));
    headers.insert("x-gp", HeaderValue::from_static("1"));
    headers
}
